...
module usbHost
  ...
  usbWires wires);
	...
	bit done_crc5; //Might not need crc dones
	...
	mux2 speSelector(.in1(inBit), .in2(sBit), .sel(spe_sel), .out(mux2_out));
	nrzi_encoder nrzi(.in1(mux2_out),.in2(in2),.out(nBit)); 
	register bitReg(.inBit(nBit), .outBit(in2), .*);
	wireCoder cable(.in(nBit),.enable(en_cable), .*);
...
//the on bit is only asserted at the very first clock cycle of the bit
//stuffing process. 
//UNSTUFF BIT MODULE {{{
...
//WIRE {{{
module wireDecoder (input tri0 pos, min,
					input bit enable,
					output logic out);
	always_comb
		if (~enable)
